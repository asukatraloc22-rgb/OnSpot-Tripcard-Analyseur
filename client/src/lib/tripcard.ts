...
const formatDate = (value: unknown, yearHint = new Date().getUTCFullYear()) => { if (!value) return ""; if (typeof value === "object") { const o = asObject(value); return formatDate(first(o.start, o.from, o.date), yearHint); } const raw = String(value).trim(); const iso = raw.match(/^(20\d{2})-(\d{2})-(\d{2})/); if (iso) { const effectiveYear = iso[1] === "2000" && yearHint !== 2000 ? String(yearHint) : iso[1]; const candidate = `${effectiveYear}-${iso[2]}-${iso[3]}`; return isoDate(candidate); } const short = raw.match(/^(\d{1,2})\s+(janv?\.?|févr?\.?|mars|avr(?:il)?\.?|mai|juin|juil?\.?|août|sept(?:embre)?\.?|oct(?:obre)?\.?|nov(?:embre)?\.?|déc(?:embre)?)$/i); if (short) { const months: Record<string, string> = { jan: "01", janv: "01", févr: "02", mars: "03", avr: "04", mai: "05", juin: "06", juil: "07", août: "08", sept: "09", oct: "10", nov: "11", déc: "12" }; const key = short[2].replace(".", "").slice(0, 4).toLowerCase(); const month = months[key] || months[key.slice(0, 3)]; if (month) return isoDate(`${yearHint}-${month}-${short[1].padStart(2, "0")}`); } return ""; };
...
const isLikelyTraveler = (value: string) => { const normalized = value.replace(/\s+/g, " ").replace(/^(?:M\.|Mr\.?|Mme|Mrs\.?|Ms\.?)\s*/i, "").trim(); const words = normalized.split(/\s+/).filter(Boolean); return normalized.length >= 4 && normalized.length <= 90 && words.length >= 2 && words.length <= 5 && !/\d|→|->|@/.test(normalized) && !/(?:hotel|hôtel|palazzo|driver|chauffeur|private|luxury|car|taxi|transfer|transfert|tour|walking|cathedral|airport|milano|milan|como|lugano|duomo|restaurant|activity|activité|voucher|room|suite|king|sedan|daytrip|shopper|call|garage|agence|agency|visages du monde|arfel)/i.test(normalized) && words.every(word => /^[A-ZÀ-Ý][A-Za-zÀ-ÿ'’-]+$/.test(word)); };
...
function enrichServiceTitles(items: ItineraryItem[], vouchers: VoucherItem[]) {
  ...
  for (const item of items) {
    ...
    if (item.type === "Transfert" && genericTitle) { const provider = corpus.match(/(?:véhicule|transferts?).{0,80}\b(ARFEL\s+TAXI)\b/i)?.[1]; if (provider) { item.title = provider; item.displayTitle = provider; item.originalTitle = provider; } }
  }
}
...
export function normalizeTripPayload(raw: unknown): TripDocument {
  ...
  enrichServiceTitles(itinerary, vouchers);
  ...
  const explicitReferences = [trip.reference, root.reference, metadata.reference, trip.cardNumber].map(value => text(value)).filter(Boolean); const numericTripId = [trip.tripId, root.tripId, metadata.tripId].map(value => text(value)).find(value => /^\d{6,}$/.test(value)); const rawReference = text(numericTripId, explicitReferences.find(value => /\btrip\s+\d{6,}/i.test(value)), explicitReferences[0], "sans référence"); const reference = /^trip\s/i.test(rawReference) ? rawReference : `Trip ${rawReference}`;
