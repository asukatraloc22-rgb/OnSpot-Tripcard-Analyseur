...
const formatDate = (value: unknown, yearHint = new Date().getUTCFullYear()) => { if (!value) return ""; if (typeof value === "object") { const o = asObject(value); return formatDate(first(o.start, o.from, o.date), yearHint); } const raw = String(value).trim(); const iso = raw.match(/^(20\d{2})-(\d{2})-(\d{2})/); if (iso) return `${iso[1]}-${iso[2]}-${iso[3]}`; const short = raw.match(/^(\d{1,2})\s+(janv?\.?|févr?\.?|mars|avr(?:il)?\.?|mai|juin|juil?\.?|août|sept?\.?|oct(?:obre)?\.?|nov(?:embre)?\.?|déc(?:embre)?)$/i); if (short) { const months: Record<string, string> = { jan: "01", janv: "01", févr: "02", mars: "03", avr: "04", mai: "05", juin: "06", juil: "07", août: "08", sept: "09", oct: "10", nov: "11", déc: "12" }; const key = short[2].replace(".", "").slice(0, 4).toLowerCase(); const month = months[key] || months[key.slice(0, 3)]; if (month) return `${yearHint}-${month}-${short[1].padStart(2, "0")}`; } return raw.slice(0, 10); };
