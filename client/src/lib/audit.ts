...
export type AuditReport = { raw: Record<string, unknown>; tripName: string; reference: string; destination: string; startDate: string; endDate: string; travelers: string[]; steps: TripStep[]; issues: AuditIssue[]; reminders: AuditReminder[]; metadata: AuditMetadata; documentChecks: DocumentCheck[]; checks: AuditCheck[]; flightDetails: FlightDetail[]; stats: { checked: number; passed: number; warnings: number; critical: number }; domains: { label: string; count: number; status: AuditStatus; note: string }[] };
...
export function analyzeTrip(raw: Record<string, unknown>): AuditReport {
  ...
  return { raw, tripName: legacy.tripName, reference: text(first(raw.reference, legacy.reference), "Dossier sans référence"), destination: legacy.destination, startDate: legacy.startDate, endDate: legacy.endDate, travelers, steps, issues, reminders, metadata, documentChecks, checks, flightDetails, stats: { checked: checks.length, passed: checks.filter((item) => item.status === "ok").length, warnings: checks.filter((item) => item.status === "pending").length, critical: checks.filter((item) => item.status === "critical").length }, domains };
}
