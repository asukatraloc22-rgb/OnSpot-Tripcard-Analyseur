...
function pnrForFlight(flight: TripStep, documents: ExportedDocument[]) { const inTitle = flight.title.match(/(?:·|\s)([A-Z0-9]{6})\b/i)?.[1]?.toUpperCase(); const number = flightNumber(flight.title); const document = documents.find((entry) => isFlightPlan(entry) && (!number || new RegExp(number.replace(" ", "\\s*"), "i").test(entry.excerpt))); const inDocument = document ? `${document.name}\n${document.excerpt}`.match(/(?:PNR|booking(?:\s+reference)?|référence(?:\s+de)?\s*(?:réservation|dossier)|confirmation(?:\s+code)?)\s*[:#-]?\s*([A-Z0-9]{5,8})/i)?.[1]?.toUpperCase() : undefined; return { pnr: inDocument ?? inTitle ?? "", evidence: inDocument ? `PNR ${inDocument} lu dans ${document?.name}.` : inTitle ? `Code ${inTitle} lu dans le segment exporté.` : "Aucun PNR ou code de réservation complet trouvé pour ce segment." }; }
