...
function parseOnSpotSteps(raw: Record<string, unknown>, all: string): { steps: TripStep[]; travelers: string[]; reference: string; destination: string; startDate: string; endDate: string; tripName: string; documents: TripStep[] } {
  ...
  const referenceSource = all.match(/Référence de réservation\s+([^\n]+)/i)?.[1]?.trim() ?? text(raw.reference, text(raw.id, "Dossier sans référence"));
  const reference = /^trip\s/i.test(referenceSource) ? referenceSource : `Trip ${referenceSource}`;
...
function analyzeLegacyTrip(raw: Record<string, unknown>): AuditReport {
  ...
  const referenceSource = text(first(meta.reference, raw.reference, meta.tripId, raw.tripId, raw.id), "Dossier sans référence"); let reference = /^trip\s/i.test(referenceSource) ? referenceSource : `Trip ${referenceSource}`; let startDate = isoDate(first(meta.startDate, trip.startDate, raw.startDate, raw.departureDate)); let endDate = isoDate(first(meta.endDate, trip.endDate, raw.endDate, raw.returnDate)); let destination = text(first(meta.destination, trip.destination, raw.destination, raw.country), "Destination à confirmer"); let tripName = text(first(meta.name, trip.name, raw.tripName, raw.title), `Dossier ${reference}`);
...
const stepMoment = (date?: string, time?: string) => {
  if (!date || date === "—" || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return null;
  const minutes = timeInMinutes(time);
  return Date.parse(`${date}T${minutes === null ? "12:00" : `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`} :00Z`.replace(" :", ":"));
};
...
export function analyzeTrip(raw: Record<string, unknown>): AuditReport {
  ...
  const documentErrors = arrayFrom(raw.documents).filter((item) => text(recordOf(item).extractionStatus, "ok") === "error");
  checks.push(check("document-extraction", "Documents", "Lecture des documents exportés", documentErrors.length ? "warning" : "ok", documentErrors.length ? `${documentErrors.length} document(s) ont échoué lors de l’extraction et ne peuvent pas servir de preuve.` : `${documents.length} document(s) exporté(s) sont lisibles par le moteur local.`, documentErrors.length ? documentErrors.map((item) => text(recordOf(item).name, "Document sans nom")).join(" · ") : documents.length ? documents.map((document) => document.name).join(" · ") : "Aucun document joint dans l’export.", documentErrors.length ? "Relire ou réexporter les documents en erreur avant de valider le dossier." : undefined));
  const outOfPeriod = steps.filter((step) => Boolean(step.date && step.date !== "—" && ((legacy.startDate !== "—" && step.date < legacy.startDate) || (legacy.endDate !== "—" && step.date > legacy.endDate))));
  checks.push(check("service-period", "Cohérence", "Prestations dans la période du voyage", outOfPeriod.length ? "critical" : "ok", outOfPeriod.length ? `${outOfPeriod.length} prestation(s) sortent de la période globale annoncée.` : "Toutes les prestations datées se situent dans la période globale exportée.", outOfPeriod.length ? outOfPeriod.map((step) => `${step.title} · ${step.date}`).join(" · ") : `Période contrôlée : ${legacy.startDate} → ${legacy.endDate}.`, outOfPeriod.length ? "Faire confirmer la période du voyage et corriger la prestation ou les dates du dossier." : undefined));
  const invalidFlightDurations = flights.filter((flight) => {
    const departure = stepMoment(flight.date, flight.time);
    const arrival = stepMoment(flight.arrivalDate, flight.arrivalTime);
    return departure !== null && arrival !== null && arrival < departure;
  });
  checks.push(check("flight-duration", "Cohérence", "Chronologie départ / arrivée des transports", invalidFlightDurations.length ? "critical" : "ok", invalidFlightDurations.length ? `${invalidFlightDurations.length} segment(s) ont une arrivée antérieure au départ.` : "Les horaires départ / arrivée disponibles sont chronologiquement cohérents.", invalidFlightDurations.length ? invalidFlightDurations.map((flight) => `${flight.title} · départ ${flight.date} ${flight.time ?? "?"} · arrivée ${flight.arrivalDate ?? "?"} ${flight.arrivalTime ?? "?"}`).join(" · ") : `${flights.filter((flight) => flight.arrivalDate && flight.arrivalTime).length}/${flights.length} vol(s) disposent d’une arrivée contrôlable.`, invalidFlightDurations.length ? "Vérifier les dates et heures locales du billet aérien puis corriger la tripcard." : undefined));
