...
export type AuditReminder = { id: string; kind: "H-24" | "CHECK-IN" | "WELCOME"; date: string; label: string; detail: string; status: AuditStatus };
...
export type AuditReport = { raw: Record<string, unknown>; tripName: string; reference: string; destination: string; startDate: string; endDate: string; travelers: string[]; steps: TripStep[]; issues: AuditIssue[]; reminders: AuditReminder[]; metadata: AuditMetadata; stats: { checked: number; passed: number; warnings: number; critical: number }; domains: { label: string; count: number; status: AuditStatus; note: string }[] };
...
export function analyzeTrip(raw: Record<string, unknown>): AuditReport {
  ...
  const addHours = (date: string, time: string | undefined, hours: number) => { const parsed = new Date(`${date}T${time || "12:00"}:00Z`); if (Number.isNaN(parsed.getTime())) return date; parsed.setUTCHours(parsed.getUTCHours() + hours); return parsed.toISOString().slice(0, 10); };
  const reminders: AuditReminder[] = steps.flatMap((step): AuditReminder[] => { if (!step.date || step.date === "—" || !/^\d{4}-\d{2}-\d{2}$/.test(step.date)) return []; if (step.type === "Vol") return [{ id: `checkin-${step.id}`, kind: "CHECK-IN" as const, date: minusOneDay(step.date), label: `Check-in · ${step.title}`, detail: `À effectuer 24 h avant le vol du ${step.date}.`, status: "pending" as AuditStatus }, { id: `welcome-${step.id}`, kind: "WELCOME" as const, date: addHours(step.date, step.time, 5), label: `Welcome call · ${step.location}`, detail: "À planifier 5 h après l’atterrissage, à confirmer avec la durée de vol et le fuseau.", status: "pending" as AuditStatus }]; if (["Transfert", "Activité"].includes(step.type)) return [{ id: `h24-${step.id}`, kind: "H-24" as const, date: minusOneDay(step.date), label: `Reconfirmation · ${step.title}`, detail: `À reconfirmer 24 h avant la prestation du ${step.date}.`, status: "pending" as AuditStatus }]; return []; });
