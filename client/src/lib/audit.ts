...
export type AuditReport = { raw: Record<string, unknown>; tripName: string; reference: string; destination: string; startDate: string; endDate: string; travelers: string[]; steps: TripStep[]; issues: AuditIssue[]; reminders: AuditReminder[]; metadata: AuditMetadata; documentChecks: DocumentCheck[]; checks: AuditCheck[]; flightDetails: FlightDetail[]; tickets: Ticket[]; stats: { checked: number; passed: number; warnings: number; critical: number }; domains: { label: string; count: number; status: AuditStatus; note: string }[] };
